...
COPY --from=fuse-overlayfs /usr/lib64/libfuse3.a                                  /pd/usr/lib64/
...
COPY --from=fuse-overlayfs /usr/lib64/pkgconfig/fuse3.pc                          /pd/usr/lib64/pkgconfig/
...
COPY --from=podman         ${LIBPOD_BASE}/policy.json                             /pd/etc/containers/
COPY --from=podman         ${LIBPOD_BASE}/registries.conf                         /pd/etc/containers/
